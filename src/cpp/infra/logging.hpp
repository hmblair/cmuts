...
[[noreturn]] void __throw_and_log(const std::string& filename, const char* file, int line,
...
#define CMUTS_LOG(level, msg)                                                                      \
    do {                                                                                           \
        if (static_cast<int>(level) >= static_cast<int>(g_log_level))                              \
            __log_level(_LOG_FILE, level, __FILE__, __LINE__, msg);                                \
    } while (0)
...
#define CMUTS_CHECK_BOUNDS(idx, size, msg)                                                         \
    do {                                                                                           \
        if (static_cast<size_t>(idx) >= static_cast<size_t>(size)) {                               \
...
#define CMUTS_CHECK_POSITIVE(val, msg)                                                             \
    do {                                                                                           \
        if ((val) <= 0) {                                                                          \
            CMUTS_THROW(std::string(msg) + " (value=" + std::to_string(val) + ")");                \
        }                                                                                          \
    } while (0)
