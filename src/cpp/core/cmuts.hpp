...
template <typename dtype, Mode mode> class DataView {
  private:
...
template <typename dtype> class Data {
  public:
    Data(BinaryFASTA& fasta, HDF5::File& hdf5, const std::string& name, bool pairwise);
...
class Stats {
  ...
  public:
    Stats(int64_t files, int64_t aligned, int64_t unaligned, int32_t references, int32_t length,
