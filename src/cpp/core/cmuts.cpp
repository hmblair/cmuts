...
template <typename dtype> Stack<dtype>::Stack(int32_t max) : _data(max) {}

template <typename dtype> void Stack<dtype>::push(dtype value) {
    _ix++;
...
template <typename dtype> size_t Stack<dtype>::size() const {
    return _ix + 1;
}

template <typename dtype> std::vector<dtype> Stack<dtype>::data() const {
    return {_data.begin(), _data.begin() + size()};
}

template <typename dtype> dtype Stack<dtype>::top() const {
    if (empty()) {
...
template <typename dtype> bool Stack<dtype>::empty() const {
    return size() == 0;
}

template <typename dtype> static inline dtype _pow(dtype base, int32_t n) {
...
static inline void _fill_at_indices(std::vector<dtype>& vec, const std::vector<int32_t>& indices,
                                    dtype value) {
...
#include <iomanip>
template <typename dtype> void _print_vector(const std::vector<dtype>& vec, int precision = 4) {
...
static inline HDF5::Memspace<dtype, N> __memspace(BinaryFASTA& fasta, HDF5::File& hdf5,
...
template <typename dtype, Mode mode> view_t<dtype, _ndims(mode)> DataView<dtype, mode>::view() {

    return _memspace.view(_offset);
}

template <typename dtype, Mode mode> void DataView<dtype, mode>::update(int32_t offset) {
...
template <typename dtype, Mode mode> int32_t DataView<dtype, mode>::size() const {

    return _memspace.size();
}

template <typename dtype>
Data<dtype>::Data(BinaryFASTA& fasta, HDF5::File& hdf5, const std::string& name, bool pairwise)
    : mods(
          __memspace<dtype, _ndims(Mode::Normal)>(fasta, hdf5, name + "/counts-1d", Mode::Normal)) {

    if (pairwise) {
        pairs = DataView<dtype, Mode::Pairwise>(__memspace<dtype, _ndims(Mode::Pairwise)>(
            fasta, hdf5, name + "/counts-2d", Mode::Pairwise));
    }
}

template <typename dtype> void Data<dtype>::update(int32_t offset) {
...
void Data<dtype>::write(int32_t offset, bool has_data, const MPI::Manager& mpi) {
    ...
    CMUTS_TRACE("Data::write: offset=" + std::to_string(offset) +
...
template <typename dtype> static inline void __joint(Data<dtype>& data) {
...
template <typename dtype>
dtype _deletion_prob(const std::vector<dtype>& mutations, const std::vector<int32_t>& indices,
                     dtype penalty) {
...
int32_t _get_ambiguous_end(int32_t start, int32_t end, const seq_t& sequence) {
...
template <typename dtype> static inline dtype _total_muts(Data<dtype>& data, int32_t ix) {
...
template <typename dtype> static inline void _normalize(std::vector<dtype>& arr, dtype sum = 1) {
...
template <typename dtype>
static inline std::vector<dtype> _spread_weights(int32_t start, int32_t end, Data<dtype>& data,
                                                 dtype mask, Spread spread) {
...
template <typename dtype>
std::vector<dtype> _enumerate_deletions(const seq_t& reference, const std::vector<dtype>& mutations,
                                        dtype penalty, dtype mask, int32_t start, int32_t end) {
...
template <typename dtype>
static inline void __match_core(Data<dtype>& data, int32_t rpos, base_t rbase, dtype mask,
                                const Params& params) {
...
template <typename dtype>
static inline void __mismatch_core(Data<dtype>& data, int32_t rpos, base_t qbase, base_t rbase,
                                   dtype mask, const Params& params) {

    if (!params.bases[rbase]) {
        mask = 0;
    }
...
template <typename dtype>
static inline void __ins_core(Data<dtype>& data, int32_t rpos, base_t qbase, dtype mask,
                              const Params& params) {
...
template <typename dtype>
static inline void __del_core(Data<dtype>& data, int32_t rpos, base_t rbase, dtype mask,
                              const Params& params) {
...
template <typename dtype> static inline void __term(Data<dtype>& data, int32_t rpos, base_t rbase) {

    data.mods.view()(rpos, rbase, IX_TERM) += 1;
};

template <typename dtype, bool CONSUMES_RPOS, bool CONSUMES_QPOS>
static inline void __match(Data<dtype>& data, HTS::CIGAR_op& op, int32_t& rpos, int32_t& qpos,
                           const seq_t& reference, const std::vector<dtype>& mask,
                           const Params& params) {

    while (op.advance()) {

        if constexpr (CONSUMES_RPOS) {
            rpos--;
        }
        if constexpr (CONSUMES_QPOS) {
            qpos--;
        }
        base_t rbase = reference[rpos];

        dtype value = mask[qpos];
        if (params.no_filter_matches) {
            value = 1;
        }
...
template <typename dtype>
static inline void __mismatch(Data<dtype>& data, HTS::CIGAR_op& op, int32_t& rpos, int32_t& qpos,
                              int32_t& last, const seq_t& reference, const std::vector<dtype>& mask,
                              const Params& params) {
...
template <typename dtype>
static inline void __ins(Data<dtype>& data, HTS::CIGAR_op& op, int32_t& rpos, int32_t& qpos,
                         int32_t& last, const seq_t& reference, const std::vector<dtype>& mask,
                         const Params& params) {

    // Skip conditions

    if (!params.insertions || op.length() > params.max_indel_length || rpos > reference.size() ||
        rpos <= 0) {
        qpos -= op.length();
        return;
    }
    ...
    dtype value = mask[qpos];
    if (params.no_filter_insertions) {
        value = 1;
    }
...
template <typename dtype>
static inline void __del_no_ambig(Data<dtype>& data, HTS::CIGAR_op& op, int32_t& rpos,
                                  int32_t& qpos, int32_t& last, const seq_t& reference,
                                  const std::vector<dtype>& mask, const Params& params) {
...
template <typename dtype>
static inline void __del_w_ambig(Data<dtype>& data, HTS::CIGAR_op& op, int32_t& rpos, int32_t& qpos,
                                 int32_t& last, const seq_t& reference,
                                 const std::vector<dtype>& mask, const Params& params) {
    ...
    std::vector<dtype> mutations = _spread_weights(start, end, data, value, params.spread);

    std::vector<dtype> deletions =
        _enumerate_deletions<dtype>(reference, mutations, penalty, value, start, rpos);
...
template <typename dtype>
static inline void __del(Data<dtype>& data, HTS::CIGAR_op& op, int32_t& rpos, int32_t& qpos,
                         int32_t& last, const seq_t& reference, const std::vector<dtype>& mask,
                         const Params& params) {
...
template <typename dtype>
static inline void __count(HTS::Alignment& aln, const seq_t& reference, Data<dtype>& data,
    ...
    if (rpos > reference.size()) {
        __throw_and_log(_LOG_FILE, "Reference position " + std::to_string(rpos) +
                                       " exceeds reference sequence length " +
    ...
    HTS::BaseMask<dtype> mask = phred.mask<dtype>(params.min_phred, params.quality_window);
...
static inline bool __check_sense(const HTS::Alignment& aln, const Params& params) {

    return (params.forward || aln.reversed) && (params.reverse || !aln.reversed);
}

static inline bool __check_mapq(const HTS::Alignment& aln, const Params& params) {

    return (aln.mapq >= params.min_mapq) && (aln.mapq < MISSING_MAPQ || params.min_mapq == 0);
}

static inline bool __check_primary(const HTS::Alignment& aln, const Params& params) {

    return (aln.primary || params.secondary);
}

static inline bool __check_length(const HTS::Alignment& aln, const Params& params) {

    return (aln.length >= params.min_length) && (aln.length <= params.max_length);
}

static inline bool __check_hamming(const HTS::Alignment& aln, const Params& params) {

    return aln.cigar.hamming() <= params.max_hamming;
}

static inline bool __check_all(const HTS::Alignment& aln, const Params& params) {

    return (aln.aligned && __check_primary(aln, params) && __check_sense(aln, params) &&
            __check_mapq(aln, params) && __check_length(aln, params) &&
            __check_hamming(aln, params));
}

template <typename dtype>
static inline bool __count_with_quality_check(HTS::Alignment& aln, const seq_t& reference,
                                              Data<dtype>& data, const Params& params,
                                              Stats& stats) {
...
template <typename dtype>
static inline void __count_reference(HTS::Iterator& iter, const seq_t& reference, Data<dtype>& data,
                                     const Params& params, Stats& stats) {
...
template <typename dtype>
static inline void __process(HTS::File& file, BinaryFASTA& fasta, HDF5::File& hdf5,
                             const MPI::Manager& mpi, const Params& params, Stats& stats,
                             Data<dtype>& data, int32_t min) {

    int32_t max = std::min(min + data.mods.size(), file.size());
...
template <typename dtype>
void run(HTS::File& file, BinaryFASTA& fasta, HDF5::File& hdf5, const MPI::Manager& mpi,
         const Params& params, Stats& stats, const std::string& name) {
    ...
    CMUTS_TRACE("run: chunk low=" + std::to_string(chunk.low) +
                " high=" + std::to_string(chunk.high) + " step=" + std::to_string(chunk.step));
    ...
    for (int32_t min = chunk.low; min < chunk.high; min += chunk.step) {
        ...
        __process<dtype>(file, fasta, hdf5, mpi, params, stats, data, min);
        iterations++;
    }
...
template void run<float>(HTS::File& file, BinaryFASTA& fasta, HDF5::File& hdf5,
                         const MPI::Manager& mpi, const Params& params, Stats& stats,
...
static inline void _print_header(int64_t files, int64_t aligned, int64_t unaligned,
                                 int64_t references, int64_t length) {
...
Stats::Stats(int64_t files, int64_t aligned, int64_t unaligned, int32_t references, int32_t length,
             double print_every, const MPI::Manager& mpi)
    : _aligned(aligned), _unaligned(unaligned), _references(references), _length(length),
      _files(files), _print_every(print_every), _mpi(mpi) {}
