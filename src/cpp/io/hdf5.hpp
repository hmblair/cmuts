...
template <typename dtype, size_t N> using arr_t = xt::xtensor<dtype, N>;
...
template <typename dtype, size_t N> class Writer;

template <typename dtype, size_t N> class Memspace;
...
class File : public H5::File {
  ...
  public:
    File(const std::string& filename, AccessMode flags, const MPI::Manager& mpi, int chunk_size,
         int compression);
    ...
    template <typename dtype, size_t N>
    Writer<dtype, N> writer(const std::vector<size_t>& dims, const std::string& name);

    template <typename dtype, size_t N>
    Memspace<dtype, N> memspace(const std::vector<size_t>& dims, const std::string& name);
};
...
template <typename dtype, size_t N> class Writer {
  ...
  public:
    Writer(File& file, const std::string& name, const std::vector<size_t>& dims, size_t _chunksize,
           int compression);

    int32_t size() const;

    void write(const arr_t<dtype, N>& data, int32_t offset) const;

    void write(const arr_t<dtype, N>& data, int32_t offset, int32_t size) const;

    void safe_write(const arr_t<dtype, N>& data, int32_t offset) const;
};
...
template <typename dtype, size_t N> class Memspace {
  ...
  public:
    Memspace(File& h5file, const std::vector<size_t>& dims, const std::string& name);
