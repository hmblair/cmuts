...
static inline std::shared_ptr<ByteStream> _get_stream(const CramBlock& block) {
    ...
    default: {
        __throw_and_log(_LOG_FILE, "Unsupported compression method \"" +
                                       std::to_string(static_cast<int32_t>(block.method)) + "\".");
    }
    }
}
...
static inline std::unique_ptr<ByteStream> _get_file_stream(const CramBlock& block) {
    ...
    default: {
        __throw_and_log(_LOG_FILE, "Unsupported file-streaming compression method \"" +
                                       std::to_string(static_cast<int32_t>(block.method)) + "\".");
    }
    }
}

template <typename dtype> static inline std::vector<dtype> _read_bgzf_array(BGZF* _bgzf_file) {
...
static inline Codec_t _codec_from_cram(int32_t value) {
    ...
    case 3: {
        return Codec_t::Huffman;
    }
    case 4: {
        return Codec_t::ByteArrayLen;
    }
    case 5: {
        return Codec_t::ByteStop;
    }
    case 6: {
        return Codec_t::Beta;
    }
    case 7: {
        return Codec_t::SubExp;
    }
    default: {
...
static inline std::string _get_codec_name(Codec_t codec) {
    ...
    default: {
        __throw_and_log(_LOG_FILE,
                        "Invalid codec \"" + std::to_string(static_cast<int32_t>(codec)) + "\".");
    }
    }
}
...
static inline CompressionMethod _get_compression(uint8_t value) {
    ...
    case 4: {
        return CompressionMethod::rans4x8;
    }
    default: {
...
CramSlice CramContainer::slice(int32_t ix) {
    ...
    if (ix >= slices.size()) {
        __throw_and_log(_LOG_FILE,
                        "The CRAM slice index (" + std::to_string(ix) + ") is out of bounds.");
    }
...
Codec::Codec(Codec_t type) : _type(type) {}

Codec::Codec(Codec_t type, int32_t id) : _type(type), _id(id) {}

ExternalCodec::ExternalCodec(int32_t id) : Codec(Codec_t::ID, id) {}
...
Codec_t Codec::type() const {
    return _type;
}

int32_t Codec::id() const {
    return _id;
}

void Codec::attach(const std::shared_ptr<ByteStream>& stream) {
    _stream = stream;
}
...
std::vector<uint8_t> ExternalCodec::array(int32_t length) {
    return _stream->bytes(length);
}

std::vector<uint8_t> ByteArrayStopCodec::array() {
    return _stream->line(_stop);
}
...
CramBlock::CramBlock(BGZF* file, bool crc)
    : _has_crc(crc), method(_get_compression(_read_bgzf_single<uint8_t>(file))),
      type(static_cast<CramBlockType>(_read_bgzf_single<uint8_t>(file))), content(_read_ITF8(file)),
      size(_read_ITF8(file)), raw(_read_ITF8(file)), file(file) {}

CramBlock::CramBlock(uint8_t*& _data, bool crc) : _has_crc(crc) {

    // The compression method and block type

    method = _get_compression(*_data);
    _data++;
    type = static_cast<CramBlockType>(*_data);
    _data++;

    // The external content identifier, compressed data size, and
    // uncompressed data size

    ITF8 _content = _decode_ITF8(_data, ITF8_MAX_SIZE);
    _data += _content.read;
    ITF8 _size = _decode_ITF8(_data, ITF8_MAX_SIZE);
    _data += _size.read;
    ITF8 _raw = _decode_ITF8(_data, ITF8_MAX_SIZE);
    _data += _raw.read;
...
template <typename dtype> std::vector<dtype> CramBlock::array() {
...
static inline ExtData_t _fc_to_enum(uint8_t value) {
    ...
    default: {
        __throw_and_log(_LOG_FILE,
                        "Invalid value \"" + std::to_string(value) + "\" in _fc_to_enum.");
    }
    }
}
...
static inline CodecMap _read_data_series(CramBlock& block) {
    ...
    for (int32_t ix = 0; ix < keys; ix++) {
        ...
        CMUTS_TRACE("Data series " + key +
                    " encoding: " + std::to_string(static_cast<int>(_get_aux_enc_type(type))));
...
CramSlice::CramSlice(uint8_t*& data, bool crc) {
    ...
    if (block.type != CramBlockType::Slice) {
        ...
        __throw_and_log(_LOG_FILE,
                        "Invalid slice header block type \"" + std::to_string(_type) + "\".");
    }
...
CramSlice::CramSlice(uint8_t*& data, CodecMap _codecs, bool crc) : CramSlice(data, crc) {
    ...
    for (int32_t ix = 0; ix < nblocks; ix++) {
        ...
        // Attach all remaining external blocks to their associated codec
            ...
            if (!codecs.externals.contains(id)) {
                continue;
            }
            ExtData_t type = codecs.externals.at(id);

            if (EXT_SKIP.contains(type)) {
                continue;
            }
...
CompressionHeader::CompressionHeader(uint8_t*& data, bool crc) {
    ...
    if (block.type != CramBlockType::Compression) {
        __throw_and_log(_LOG_FILE, "Invalid compression header block type \"" +
                                       std::to_string(static_cast<int32_t>(block.type)) + "\".");
    }
...
CodecMap CompressionHeader::codecs() const {
    return _codecs;
}
...
bool CompressionHeader::delta() const {
    return _map.AP;
}
...
CramContainerBase::CramContainerBase(BGZF* file, int32_t version)
    : _file(file), _has_crc(version >= 30), ptr(bgzf_tell(file)),
      size(_read_bgzf_single<int32_t>(file)), reference(_read_ITF8(file)), offset(_read_ITF8(file)),
      span(_read_ITF8(file)), records(_read_ITF8(file)),
      counter(_has_crc ? _read_LTF8(file) : _read_ITF8(file)), bases(_read_LTF8(file)),
      blocks(_read_ITF8(file)), landmarks(_read_ITF8_array(file)) {
...
bool CramContainerBase::eof() const {
    ...
    return ((offset == CRAM_EOF_POS) && (reference == UNALIGNED));
}
...
CramContainer::CramContainer(BGZF* file, int32_t version) : CramContainerBase(file, version) {
    ...
    if (records > 0) {
        _comp = CompressionHeader(ptr, _has_crc);
        ix++;
    }
...
CramHeaderContainer::CramHeaderContainer(BGZF* file, int32_t version)
    ...
    if (block.type != CramBlockType::Header) {
        __throw_and_log(_LOG_FILE, "Invalid file header block type \"" +
                                       std::to_string(static_cast<int32_t>(block.type)) + "\".");
    }
...
CodecMap CramContainer::codecs() const {
    return _comp.codecs();
}
...
bool CramContainer::delta() const {
    return _comp.delta();
}
...
CramIterator::CramIterator(BGZF* file, int32_t reads, int32_t version)
    : Iterator(reads), _file(file), _version(version), _container(file, _version),
...
bool CramIterator::empty() const {
    return _remaining_in_slice <= 0;
}
...
void CramIterator::to(int32_t reference) {
    ...
    if (_reads == 0 || !_container.multi()) {
        return;
    }

    _tmp = next();
    while (_tmp.reference != reference && !end()) {
        _tmp = next();
        _curr--;
    }
    _use_tmp = true;
...
Alignment CramIterator::next() {
    ...
    CMUTS_THROW_IF(static_cast<int32_t>(scores.size()) != length,
                   "CRAM quality scores size (" + std::to_string(scores.size()) +
                       ") does not match read length (" + std::to_string(length) + ")");
    PHRED phred(scores);

    return Alignment::create(aligned, primary, reversed, mapq, length, _offset, reference,
                             std::move(_cigar), std::move(phred));
}
...
CramFile::CramFile(const std::string& name) : File(name, FileType::CRAM) {
