...
The reactivity profiles are read from the HDF5 written by `profiles.py` (one
...
# Datasets in the profiles HDF5 that the accuracy benchmark scores: cmuts at its
# three spread modes plus the two existing tools. Generated by profiles.py.
...
def main() -> None:
    ...
    # Load each (dataset, condition) reactivity array (FASTA order), keyed by name.
    ...
    with h5py.File(args.profiles, "r") as f:
        for dataset in DATASETS:
            for cond in CONDITIONS:
                ...
                if key in f:
                    arr = f[key][:]
                    profiles[(dataset, cond)] = {
                        names[i]: arr[i] for i in range(min(len(names), arr.shape[0]))
                    }
    if not profiles:
        ap.error(f"none of the expected datasets {DATASETS} found in {args.profiles}")
