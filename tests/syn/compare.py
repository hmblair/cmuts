...
with h5py.File(CMUTS_FILE, "r") as cmuts_f, h5py.File(EXPECTED_FILE, "r") as exp_f:
    ...
    cmuts_profile = np.nan_to_num(cmuts_muts / cmuts_cov, 0.0, 0.0, 0.0)
    exp_profile = np.nan_to_num(exp_muts / exp_cov, 0.0, 0.0, 0.0)
    ...
    if USE_RF:
        ...
        rf_profile = np.nan_to_num(rf_muts / rf_cov, 0.0, 0.0, 0.0)
