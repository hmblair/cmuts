...
- [ ] `NormConfig.norm_cutoff` / `norm_percentile` are defined and collected from the form but unused (not mapped into the normalization options).
- [ ] `no_insertions` is shared between `CoreConfig` and `NormConfig` via one form field — implicit coupling.
