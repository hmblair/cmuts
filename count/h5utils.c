...
Filespace makeFilespace(
    ...
    // Set the file access property list for MPI
    H5Pset_fapl_mpio(faplID, comm, info);

    // Create the file with parallel I/O support
    filespace.fileID = H5Fcreate(
        filename,
        H5F_ACC_TRUNC,
        H5P_DEFAULT,
        faplID
...
Filespace openFilespace(
    ...
    // Open the file with parallel I/O support
    filespace.fileID = H5Fopen(
        filename,
        flags,
        faplID
    );
...
void writeSlab(
    ...
    // Create a property list for parallel I/O
